...
export function registerGameOverHandlers(socket, io) {
  socket.on(
    ...
    async ({ gameId, playerRole }) => {
      logger.info(
        `[Game ID: ${gameId}] Received ${GAME_EVENTS.ACTION_REQUEST_NEW_GAME} from ${playerRole} (socket ${socket.id})`,
      );
      try {
        ...
        if (!gameState) {
          ...
          logger.error(
            `[Game ID: ${gameId}] Game not found for ${GAME_EVENTS.ACTION_REQUEST_NEW_GAME}`,
          );
          return;
        }
        ...
        if (gameState.gamePhase !== GAME_PHASES.GAME_OVER) {
          ...
          logger.warn(
            `[Game ID: ${gameId}] ${GAME_EVENTS.ACTION_REQUEST_NEW_GAME} called when game phase is ${gameState.gamePhase}`,
          );
          return;
        }
        ...
        logger.info(
          `[Game ID: ${gameId}] Emitted ${GAME_EVENTS.GAME_STATE_UPDATE} for new game. New state game ID: ${newGameState.gameId}. Phase: ${newGameState.gamePhase}`,
        );

        if (newGameState.gameId !== gameId) {
          logger.warn(
            `[Game ID: ${gameId}] Game was reset, and a new gameId ${newGameState.gameId} was generated. Clients in room ${gameId} received the new state. Ensure client handles potential gameId change.`,
          );
        ...
        logger.error(
          `[Game ID: ${gameId}] Error in ${GAME_EVENTS.ACTION_REQUEST_NEW_GAME} handler: ${error.message}`,
        );
