...
For all unit tests using `esmock`, ensure that:
1.  Test files use `import.meta.url` when calling `esmockWithPaths`.
2.  Mock keys exactly match the import strings used *within the source module being tested* (e.g., `../../utils/deck.js` not `../../../src/utils/deck.js`).
3.  The `esmockWithPaths` wrapper (from `test/utils/esmockWrapper.js`) is used for consistent and cross-platform path resolution.
...
*   **`src/utils/players.js` - 65%**
    *   **To-Do:** The existing tests for `initializePlayers` are a good start, but critical functions are untested.
        ...
        2.  **Expand Unit Tests:** Add tests for `isTeammate`, `getPartner`, `getPlayerBySocketId`, `getRoleBySocketId`, and the enhanced `getNextPlayer` (including the "skip disconnected" logic). When testing, use `esmockWithPaths` if `players.js` itself imports other internal modules that need mocking.
...
*   **`src/utils/deck.js` - 50%**
    *   **To-Do:** This is a high-priority gap. The logic is critical but completely untested.
        ...
        3.  Add tests for `createDeck`, `shuffleDeck`, `isRightBower`, `isLeftBower`, and `sortHand`. Use `esmockWithPaths` for the test setup if `deck.js` has any internal dependencies requiring mocks.
...
*   **`src/utils/lobbyUtils.js` - 50%**
    *   **To-Do:** The pure functions for lobby management are untested.
        ...
        2.  Add tests for `assignRoleToPlayer`, `isLobbyFull`, and `getNextAvailableRole` covering various lobby states. Use `esmockWithPaths` for the test setup if `lobbyUtils.js` has any internal dependencies requiring mocks.
...
*   **`src/utils/logger.js` - 50%**
    *   **To-Do:** The logger's initialization logic is untested.
        ...
        3.  Test the `log` compatibility wrapper to ensure it calls the correct underlying `pino` methods. This will require mocking `pino` and `process.env`, so using `esmockWithPaths` is essential to correctly mock these dependencies.
...
*   **`src/game/logic/aiLogic.js` - 0% (Missing File)**
    *   **To-Do:** This is a critical missing piece for a functional online game.
        ...
        3.  Create a corresponding `test/game/logic/aiLogic.unit.test.js` to validate the AI's decisions under various scenarios. Mocking internal dependencies (e.g., card evaluation functions, external constants) using `esmockWithPaths` will be crucial.
...
*   **`src/utils/settingsUtils.js` - 0% (Missing File)**
    *   **To-Do:** A full game needs configurable rules.
        ...
        3.  Create a corresponding `test/utils/settingsUtils.unit.test.js`. Use `esmockWithPaths` for the test setup if `settingsUtils.js` has any internal dependencies requiring mocks.
...
*   **`src/utils/statsUtils.js` - 0% (Missing File)**
    *   **To-Do:** While not essential for gameplay, stats are a core feature of card games. The pure calculation logic belongs in Layer 1.
        ...
        3.  Create a corresponding `test/utils/statsUtils.unit.test.js`. Use `esmockWithPaths` for the test setup if `statsUtils.js` has any internal dependencies requiring mocks.
...
*   **`src/utils/historyUtils.js` - 0% (Missing File)**
    *   **To-Do:** To provide a clear game log, a dedicated utility is needed to format game events into human-readable messages.
        ...
        3.  Create a corresponding `test/utils/historyUtils.unit.test.js`. Use `esmockWithPaths` for the test setup if `historyUtils.js` has any internal dependencies requiring mocks (e.g., a `Date` object for timestamps).
...
*   **`src/utils/idGenerator.js` - 0% (Missing File)**
    *   **To-Do:** Replace ad-hoc ID generation with a centralized, robust utility.
        ...
        2.  Implement `generateGameId()` to create a more reliable unique ID than the current method (likely using `nanoid`).
        3.  Create a corresponding `test/utils/idGenerator.unit.test.js`. This will require mocking the external `nanoid` library using `esmockWithPaths`.
...
*   **`src/config/locales/en.json` & `src/utils/i18n.js` - 0% (Missing Files)**
    *   **To-Do:** For a production-quality game, all user-facing strings should be externalized for internationalization.
        ...
        3.  Create a corresponding `test/utils/i18n.unit.test.js`. Mocking the `en.json` import using `esmockWithPaths` will be essential for testing different locale data.