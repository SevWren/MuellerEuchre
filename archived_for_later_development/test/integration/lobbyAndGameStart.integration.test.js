...
const playerConnectionHandlersModulePath =
...
const mockGameRepository = {
  getGame: sinon.stub().callsFake(async (gameId) => {
    return memoryGameStore[gameId]
      ? JSON.parse(JSON.stringify(memoryGameStore[gameId]))
      : null;
  }),
...
const setupMockIo = () => {
  mockIoInstance = {
      ...
      if (!mockIoInstance._roomSpies[roomOrSocketId]) {
        const emitSpy = sinon.spy((event, ...args) => {
          ...
          for (const socket of mockIoInstance.sockets.values()) {
            if (
              socket.rooms.has(roomOrSocketId) ||
              socket.id === roomOrSocketId
            ) {
    ...
    getRoomBroadcasts: (roomId, eventName) => {
      ...
      return roomEmitSpy
        ? roomEmitSpy.getCalls().filter((call) => call.args[0] === eventName)
        : [];
    },
  };
};
...
describe("Lobby and Game Start Integration Tests", () => {
  ...
  beforeEach(async () => {
    ...
    const commonMocks = {
      ...
      "../../src/game/state.js": {
        ...
        updateGameState: sinon.stub().callsFake((updaterFn) => {
          ...
          stubbedLogger.debug(
            "[Integration Test Mock] updateGameState called, preventing real call.",
          );
        ...
        createInitialGameState: sinon.stub().callsFake((gameId) => {
          ...
          stubbedLogger.debug(
            "[Integration Test Mock] createInitialGameState called.",
          );
      ...
      socket.on(GAME_EVENTS.JOIN_GAME, async (data, ack) => {
        ...
        try {
          if (!gameId) {
            ...
            gameState.players[assignedRole] = {
              id: user.id,
              name: playerName,
              socketId: socket.id,
              isConnected: true,
              role: assignedRole,
              teamId:
                PLAYER_ROLES.indexOf(assignedRole) % 2 === 0 ? "NS" : "EW",
              isActive: true,
              tricksWonThisHand: 0,
              score: 0,
            };
            ...
            if (gameState.gamePhase !== GAME_PHASES.LOBBY) {
              return ack({
                status: "error",
                message: `Game ${gameId} is not in lobby phase.`,
              });
            }
            ...
            for (const role of PLAYER_ROLES) {
              if (
                ...
                gameState.players[role].id === user.id &&
          ...
          socket.emit(GAME_EVENTS.ASSIGN_ROLE, {
            ...
            isHost: gameState.hostId === user.id,
            playerId: user.id,
          });
          ...
          if (
            activePlayers === 4 &&
            gameState.gamePhase === GAME_PHASES.LOBBY
          ) {
            ...
            return ack(null, {
              status: "ok",
              message: "Joined game, and game is now starting.",
              gameId: gameId,
              role: assignedRole,
              players: stateAfterAutoStart.players,
              gameState: stateAfterAutoStart,
            });
          } else {
          ...
          return ack({
            status: "error",
            message:
              error.message || "An error occurred while joining the lobby.",
          });
        }
      });
      ...
      socket.on("request_start_game", async (data, ack) => {
        ...
        try {
          ...
          if (!gameState) {
            return ack({
              status: "error",
              message: "Game not found. Cannot start.",
            });
          }
          if (
            Object.values(gameState.players).filter((p) => p.isActive).length <
            4
          ) {
            return ack({
              status: "error",
          ...
          return ack(null, {
            ...
            gameState: stateAfterDealing,
          });
        } catch (error) {
          console.error(
            `[Mock Lobby Handler Error - request_start_game] ${error.message}`,
            error,
          );
          return ack({
            status: "error",
    ...
    const playerConnectionHandlersModule = await esmock(
      playerConnectionHandlersModulePath,
      commonMocks,
    );
    esmocked_handlePlayerDisconnect =
      playerConnectionHandlersModule.handlePlayerDisconnect;
    esmocked_handleRejoinGame = playerConnectionHandlersModule.handleRejoinGame;

    clients = [];
    for (let i = 0; i < 4; i++) {
      const clientSocket = new MockSocket(`clientSocketId-${i + 1}`, {
        id: `mockUser${i + 1}`,
      });
      ...
      clientSocket.on(GAME_EVENTS.RECONNECT, async (data, ack) => {
        await esmocked_handleRejoinGame(
          clientSocket,
          mockIoInstance,
          data.gameId,
          data.playerId,
  ...
  it("Test Case: Successful 4-Player Game Start and Entry into Bidding", async () => {
    ...
    // Removed console.log
    await client1._simulateClientEmit(
      GAME_EVENTS.JOIN_GAME,
      { gameIdToJoin: null, playerName: "Player 1" },
      client1Ack,
    );
    ...
    if (!ackArgs1) {
      ...
      throw new Error(
        `ackArgs1 is undefined. Ack was likely called with error: ${JSON.stringify(client1Ack.getCall(0).args[0])}`,
      );
    }
    ...
    for (let i = 1; i < 4; i++) {
      ...
      // Removed console.log
      await client._simulateClientEmit(
        GAME_EVENTS.JOIN_GAME,
        { gameIdToJoin: assignedGameId, playerName: `Player ${i + 1}` },
        clientAck,
      );
      ...
      if (!ackArgsClient)
        throw new Error(
          `ackArgsClient is undefined for Player ${i + 1}. Ack error: ${JSON.stringify(clientAck.getCall(0).args[0])}`,
        );
      ...
      expect(
        clientAssignRoleEmit,
        `ASSIGN_ROLE not emitted for client ${i + 1}`,
      ).to.exist;
      // Removed console.log
    ...
    expect(
      Object.keys(gameState.players).filter(
        (p) => gameState.players[p].isActive,
      ).length,
    ).to.equal(4);
    ...
    const dateStub = sinon
      ...
      .returns(handStartDate);

    let biddingReadyGameState = gameState;

    dateStub.restore();

    expect(biddingReadyGameState.turnCard)
      .to.be.an("object")
      .with.keys("suit", "value", "id", "name");
    expect(biddingReadyGameState.kitty.length).to.equal(24 - 20 - 1);

    const dealer = biddingReadyGameState.dealer;
    const expectedActiveBidder =
      PLAYER_ROLES[(PLAYER_ROLES.indexOf(dealer) + 1) % 4];
    ...
    for (let i = 0; i < 4; i++) {
      ...
      expect(biddingReadyGameState.players[playerRole].name).to.equal(
        `Player ${i + 1}`,
      );
    }
    ...
    const lastMessage =
      biddingReadyGameState.gameMessages[
        biddingReadyGameState.gameMessages.length - 1
      ];
    expect(lastMessage.text).to.include(
      `New hand started. Dealer is ${dealer}. ${biddingReadyGameState.turnCard.id} is up. ${expectedActiveBidder} to make the first bid`,
    );

    const roomBroadcasts = mockIoInstance.getRoomBroadcasts(
      assignedGameId,
    ...
    expect(finalGameStateBroadcast.gamePhase).to.equal(
      GAME_PHASES.ORDER_UP_ROUND1,
    );

    clients.forEach((client) => {
      const clientStateUpdateCall = client.getLastEmit(
        GAME_EVENTS.STATE_UPDATE,
      );
      expect(
        clientStateUpdateCall,
        `Client ${client.id} did not receive STATE_UPDATE`,
      ).to.exist;
      // Removed console.log
      ...
      if (client.playerRole)
        expect(
          clientStateUpdate.players[client.playerRole].hand.length,
        ).to.equal(5);
      expect(clientStateUpdate.turnCard).to.deep.equal(
        biddingReadyGameState.turnCard,
      );
    });
  });
  ...
  it("Test Case: 4 Players Join, One Explicitly Starts Game, Hands are Dealt", async () => {
    ...
    if (!ackArgs1)
      throw new Error(
        `ackArgs1 is undefined. Ack error: ${JSON.stringify(client1Ack.getCall(0).args[0])}`,
      );
    expect(ackArgs1.status).to.equal("ok");
    assignedGameId = ackArgs1.gameId;
    client1.gameId = assignedGameId;
    ...
    for (let i = 1; i < 4; i++) {
      ...
      if (i < 3) {
        // For players 2 and 3 joining
        expect(currentGameState.gamePhase).to.equal(GAME_PHASES.LOBBY);
      } else {
        // After player 4 joins, the other test implies it auto-starts.
        ...
        if (currentGameState.gamePhase === GAME_PHASES.LOBBY) {
          ...
          console.log(
            `[Test DEBUG] Game phase reset to LOBBY for explicit start test. Game ID: ${assignedGameId}`,
          );
        }
      }
    }
    ...
    if (!ackArgsStart)
      throw new Error(
        `ackArgsStart is undefined. Ack error: ${JSON.stringify(requestStartAck.getCall(0).args[0])}`,
      );
    ...
    expect(startedGameState.turnCard)
      .to.be.an("object")
      .with.keys("suit", "value", "id", "name");
    ...
    for (let i = 0; i < 4; i++) {
      const playerRole = PLAYER_ROLES[i];
      expect(startedGameState.players[playerRole].hand.length).to.equal(
        5,
        `Player ${playerRole} hand length incorrect`,
      );
      expect(startedGameState.players[playerRole].name).to.equal(
        `Player ${i + 1}`,
      );
    }
    ...
    expect(
      finalGameStateBroadcast.players[PLAYER_ROLES[0]].hand.length,
    ).to.equal(5);
  ...
  it("Test Case: Player Leaves Lobby Before Game Starts", async () => {
    ...
    if (!ackArgs1)
      throw new Error(
        `ackArgs1 is undefined for Player 1. Ack error: ${JSON.stringify(client1Ack.getCall(0).args[0])}`,
      );
    assignedGameId = ackArgs1.gameId;
    client1.gameId = assignedGameId;
    client1.playerRole = ackArgs1.role;
    ...
    await client2._simulateClientEmit(
      GAME_EVENTS.JOIN_GAME,
      { gameIdToJoin: assignedGameId, playerName: "Player 2" },
      client2Ack,
    );
    const ackArgs2 = client2Ack.getCall(0).args[1];
    if (!ackArgs2)
      throw new Error(
        `ackArgs2 is undefined for Player 2. Ack error: ${JSON.stringify(client2Ack.getCall(0).args[0])}`,
      );
    client2.gameId = assignedGameId;
    client2.playerRole = ackArgs2.role;

    let client3Ack = sinon.spy();
    await client3._simulateClientEmit(
      GAME_EVENTS.JOIN_GAME,
      { gameIdToJoin: assignedGameId, playerName: "Player 3" },
      client3Ack,
    );
    const ackArgs3 = client3Ack.getCall(0).args[1];
    if (!ackArgs3)
      throw new Error(
        `ackArgs3 is undefined for Player 3. Ack error: ${JSON.stringify(client3Ack.getCall(0).args[0])}`,
      );
    ...
    await esmocked_handlePlayerDisconnect(
      client3,
      mockIoInstance,
      assignedGameId,
    );
    ...
    expect(updatedGameState.players[client3.playerRole].isConnected).to.be
      .false;
    ...
    const finalGameStateInRepo =
      await mockGameRepository.getGame(assignedGameId);
    expect(finalGameStateInRepo.players[client3.playerRole].isConnected).to.be
      .false;
    ...
    [client1, client2].forEach((client) => {
      ...
      expect(
        clientStateUpdateCall,
        `Client ${client.id} did not receive STATE_UPDATE after disconnect`,
      ).to.exist;
      const clientStateUpdate = clientStateUpdateCall.args[1];
      // The broadcasted state should reflect the disconnected status
      expect(clientStateUpdate.players[client3.playerRole].isConnected).to.be
        .false;
    });
  });
  ...
  it("Test Case: Attempt to Join Full Lobby/Game", async () => {
    ...
    for (let i = 0; i < 4; i++) {
      let clientAck = sinon.spy();
      // Removed console.log
      await clients[i]._simulateClientEmit(
        GAME_EVENTS.JOIN_GAME,
        { gameIdToJoin: assignedGameId, playerName: `Player ${i + 1}` },
        clientAck,
      );
      ...
      if (i === 0 && !assignedGameId) {
        ...
        if (!ackArgsLoop1)
          throw new Error(
            `ackArgsLoop1 is undefined for Player ${i + 1}. Ack error: ${JSON.stringify(ackCall.args[0])}`,
          );
      ...
      if (!ackArgsLoop)
        throw new Error(
          `ackArgsLoop is undefined for Player ${i + 1}. Ack error: ${JSON.stringify(ackCall.args[0])}`,
        );
    ...
    expect(
      Object.keys(currentGameState.players).filter(
        (p) => currentGameState.players[p].isActive,
      ).length,
    ).to.equal(4);
    ...
    // Removed console.log
    await client5._simulateClientEmit(
      GAME_EVENTS.JOIN_GAME,
      { gameIdToJoin: assignedGameId, playerName: "Player 5" },
      client5Ack,
    );

    expect(client5Ack).to.have.been.calledOnce;
    const ackArgs = client5Ack.getCall(0).args[0];
    expect(ackArgs).to.deep.equal({
      status: "error",
      message: `Game ${assignedGameId} is not in lobby phase.`,
    });
  });
});
