...
export function attemptToStartGame(currentGameState, requestingPlayerRole) {
  logger.debug({ gameId: currentGameState?.gameId, requestingPlayerRole }, 'Attempting to start game from lobby.');

  if (!currentGameState || !currentGameState.players || !requestingPlayerRole) {
    logger.warn({ currentGameState, requestingPlayerRole }, 'ValidationError: Missing currentGameState or requestingPlayerRole to start game.');
  ...
  if (currentGameState.gamePhase !== GAME_PHASES.LOBBY) {
    logger.warn({ gameId: currentGameState.gameId, currentPhase: currentGameState.gamePhase }, `InvalidPhaseError: Game cannot be started from ${currentGameState.gamePhase} phase. Must be in LOBBY phase.`);
  ...
  if (connectedPlayers < requiredPlayers) {
    logger.warn({ gameId: currentGameState.gameId, connectedPlayers, requiredPlayers }, `PhaseLogicError: Not enough players to start. Need ${requiredPlayers}, have ${connectedPlayers}.`);
  ...
  logger.info({ gameId: newState.gameId, newPhase: newState.gamePhase }, 'Game successfully transitioned to DEALING phase.');
  return { success: true, updatedGameState: newState, message: 'Game successfully transitioned to DEALING phase.' };
}
