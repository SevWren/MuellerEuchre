  ...
  io.on("connection", (socket) => {
    ...
    socket.on(GAME_EVENTS.RECONNECT, (data) => {
      if (data && data.gameId && data.playerId) {
        logger.info(
          { socketId: socket.id, gameId: data.gameId, playerId: data.playerId },
          `Received ${GAME_EVENTS.RECONNECT} request.`,
        );
        ...
        logger.warn(
          ...
          `Invalid data for ${GAME_EVENTS.RECONNECT}. 'gameId' and 'playerId' are required.`,
        );
        socket.emit(GAME_EVENTS.ERROR, {
          message:
            "Rejoin request failed: 'gameId' and 'playerId' are required.",
        });
      }
    });
