## Euchre Multiplayer Rewrite - LLM Prompt Template (Single Task Focus)
...
### 3. The `todo.md` Task Backlog

The `todo.md` file is our task backlog. It is a prioritized list of concrete tasks derived directly from the layered plan in `Thursday-Jules.txt`.
...
*   **Your Role:** You will be provided with a **specific high-priority task** from this file to implement. After completing the task, you will update its status in `todo.md`.

---

### 4. Your Task for This Session

Your task is to implement a **single, high-priority feature/fix** by following these steps precisely.

**1. Implement Task:**
    *   Implement the **specified high-priority task.** The details of this task (e.g., its description from `todo.md` or a direct instruction) will be provided to you as the primary input for this session.
...
**3. Update `todo.md` with Progress:**
    ...
    *   Move the **task** you just completed to the bottom of the file (within the completed tasks section).
    *   Mark **it** as complete (e.g., `[ ]` -> `[x]`).
    *   Below **the** completed task, add a concise, one-line summary of the implementation (e.g., `> Implemented asynchronous file and console transports for the logger.`)

**4. Suggest Next Logical Task (Optional):**
    *   After completing your assigned task and its tests, briefly consult the layered plan in `Thursday-Jules.txt` and the remaining tasks in `todo.md`.
    *   If a single, clear next logical task emerges as high priority based on your completed work and the layered plan, you may suggest it. **Do not add it to `todo.md` yourself.** This suggestion is for human review.
    *   If no single task is an obvious immediate successor, or if you are unsure, you can state that next task prioritization is deferred.

**5. Final Output:**
    *   Your final response should include all the new/modified code files related to the completed task, and the single, updated `todo.md` file (reflecting only the completion of the assigned task).
