...
describe("UiService", () => {
  ...
  beforeEach(() => {
    ...
    mockStateService = {
      getPlayerHand: sinon
        .stub()
        .returns([{ id: "AH", rank: "A", suit: "HEARTS" }]),
      getTurnCard: sinon
        .stub()
        .returns({ id: "KD", rank: "K", suit: "DIAMONDS" }),
    ...
    mockSocketService = {
      emitOrderUpDecision: sinon
        .stub()
        .resolves({ status: "ok", message: "Decision made" }),
      emitDealerDiscard: sinon
        .stub()
        .resolves({ status: "ok", message: "Card discarded" }),
      emitCallTrumpDecision: sinon
        .stub()
        .resolves({ status: "ok", message: "Trump called" }),
      emitGoAloneDecision: sinon
        .stub()
        .resolves({ status: "ok", message: "Go alone decided" }),
      emitPlayCard: sinon
        .stub()
        .resolves({ status: "ok", message: "Card played" }),
      emitRejoinGame: sinon
        .stub()
        .resolves({ status: "ok", message: "Rejoined game" }),
  ...
  describe("Error and Message Display", () => {
    it("displayGlobalError should call displayMessage with error type", () => {
      ...
      expect(uiService.displayMessage).to.have.been.calledOnceWith(
        "Global Error: Test global error",
        "error",
      );
    });
    ...
    it("showErrorModal should log to console.error (conceptual)", () => {
      ...
      expect(console.error).to.have.been.calledWith(
        sinon.match(
          /Showing error modal \(title: Test Title\): "Test error modal"/,
        ),
      );
    });
    ...
    it("displayMessage should log to console.log (conceptual)", () => {
      ...
      expect(console.log).to.have.been.calledWith(
        sinon.match(/Displaying message \(type: info\): "Test info message"/),
      );
    });
  });
  ...
  describe("Action-Triggering Methods", () => {
    it("promptOrderUp should call socketService.emitOrderUpDecision and handle promise", async () => {
      ...
      expect(uiService.showSpinner).to.have.been.calledOnceWith(
        "Submitting your decision...",
      );
      expect(mockSocketService.emitOrderUpDecision).to.have.been.calledOnceWith(
        true,
      );
    ...
    it("promptOrderUp should handle rejection from socketService", async () => {
      ...
      try {
        ...
        expect.fail(
          "promptOrderUp should have thrown or returned a rejected promise",
        );
      } catch (e) {
    ...
    it("handlePlayCardSelection should call socketService.emitPlayCard for valid card", async () => {
      ...
      expect(uiService.showSpinner).to.have.been.calledOnce;
      expect(mockSocketService.emitPlayCard).to.have.been.calledOnceWith(
        null,
        null,
        card,
      );
      expect(uiService.hideSpinner).to.have.been.calledOnce;
      expect(uiService.displayMessage).to.have.been.calledOnceWith(
        sinon.match(/You played A of HEARTS/),
        "success",
      );
    });
    ...
    it("handlePlayCardSelection should show error for invalid card and not call socketService", async () => {
      ...
      try {
        ...
        expect.fail(
          "handlePlayCardSelection should have rejected due to invalid card",
        );
      } catch (error) {
  ...
  describe("Reconnection UI Flow Methods", () => {
    it("promptForRejoin (user confirms) should call socketService.emitRejoinGame", async () => {
      ...
      expect(uiService.showSpinner).to.have.been.calledOnceWith(
        "Rejoining game game789...",
      );
      expect(mockSocketService.emitRejoinGame).to.have.been.calledOnceWith(
        "game789",
      );
    ...
    it("showConnectionLostMessage should call displayMessage with error type", () => {
      ...
      expect(uiService.displayMessage).to.have.been.calledOnceWith(
        "Network down Attempting to reconnect...",
        "error",
      );
    });
  ...
  describe("Display Methods (Conceptual)", () => {
    it("displayPlayerHand should call stateService.getPlayerHand and log conceptually", () => {
      mockStateService.getPlayerHand.returns([
        { rank: "A", suit: "SPADES" },
        { rank: "K", suit: "SPADES" },
      ]);
    ...
    it("displayTurnCard should call stateService.getTurnCard and log conceptually", () => {
      ...
      expect(console.log).to.have.been.calledWith(
        sinon.match(/Displaying Turn Card: Q of HEARTS/),
      );
    });
  });
});
