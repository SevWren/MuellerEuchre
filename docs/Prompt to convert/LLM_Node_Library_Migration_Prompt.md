...
### **Output Requirements**
-   Provide the fully refactored and corrected code for the relevant file(s).
-   Clearly explain the changes you made and *why* you made them. Reference the legacy code you replaced and the specific `node:test` patterns you implemented, especially the Dynamic Import Pattern and any path corrections.
-   Explicitly state which tasks from the workflow you completed.
-   If you identify a new problem or ambiguity, clearly state it and ask for guidance before proceeding.