...
import { cardToString, isLeftBower, isRightBower } from './deck.js';
...
// Add these exports at the end of the file (after serverIsValidPlay)
export function validateGameState(gameState) {
    ...
    if (gameState.players && Object.keys(gameState.players).length < 4) throw new Error('Invalid player count');
    if (gameState.gamePhase && !['LOBBY', 'PLAYING_TRICKS', 'SCORING', 'GAME_OVER'].includes(gameState.gamePhase)) throw new Error('Invalid game phase');
    if (gameState.currentPlayer && gameState.players && !gameState.players[gameState.currentPlayer]) throw new Error('Invalid current player');
}

export function validatePlayerAction(action) {
    // Placeholder: implement real validation logic as needed
    if (!action || typeof action !== 'object') throw new Error('Invalid action');
    if (action.type === 'play' && (!action.card || !action.card.suit || !action.card.value)) throw new Error('Invalid card');
}

export function validateBid(bid, gameState) {
    // Placeholder: implement real validation logic as needed
    if (!bid || typeof bid !== 'object') throw new Error('Invalid bid');
    if (!gameState || typeof gameState !== 'object') throw new Error('Invalid game state');
    if (bid.player !== gameState.currentPlayer) throw new Error('Not your turn');
    if (!['orderUp', 'pass', 'callTrump'].includes(bid.type)) throw new Error('Invalid bid type');
}
