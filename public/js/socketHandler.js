...
class SocketHandler {
  ...
  initializeSocketEvents() {
    ...
    this.socket.on("cardPlayed", (data) => {
      this.showMessage(
        `${data.playerName} played ${this.formatCard(data.card)}`,
      );
    ...
    this.socket.on("handStarted", (data) => {
      this.showMessage(
        `New hand started. Dealer: ${data.dealerName}, Trump: ${data.trumpSuit}`,
      );
    ...
    this.socket.on("gameOver", (data) => {
      this.showMessage(
        `Game over! ${data.winningTeam} wins ${data.score}-${data.opponentScore}!`,
      );
  ...
  joinGame(playerName) {
    ...
    if (this.role) {
      throw new Error(
        "Already in a game. Please leave the current game before joining another.",
      );
    }
    ...
    try {
      this.socket.emit("joinGame", { name: trimmedName }, (response) => {
        ...
        this.showMessage(
          `Joined as ${trimmedName} (${response.role})`,
          "success",
        );
  ...
  playCard(cardId) {
    ...
    if (!validCard) {
      throw new Error(
        `Invalid card ID format: ${cardId}. Expected format: [Rank][Suit] (e.g., 'AH' for Ace of Hearts)`,
      );
    }
  ...
  makeBid(bidType, options = {}) {
    ...
    if (!validBidTypes.includes(bidType)) {
      throw new Error(
        `Invalid bid type: ${bidType}. Must be one of: ${validBidTypes.join(", ")}`,
      );
    }
    ...
    if (options.suit) {
      ...
      if (!validSuits.includes(options.suit.toLowerCase())) {
        throw new Error(
          `Invalid suit: ${options.suit}. Must be one of: ${validSuits.join(", ")}`,
        );
      }
    }
