...
import {
  ...
  getGameState,
  updateGameState,
} from "../../game/state.js"; // Import state management functions
...
function assignRoleToPlayer(gameState, role, userId, playerName, socketId) {
  ...
  if (newGameState.players[role].tricksWonThisHand === undefined)
    newGameState.players[role].tricksWonThisHand = 0;
  if (newGameState.players[role].score === undefined)
    newGameState.players[role].score = 0;
...
export function registerLobbyHandlers(socket, io) {
  socket.on("request_start_game", async (data, ack) => {
    ...
    if (!data || !data.gameId) {
      logger.warn(
        ...
        "Invalid data for request_start_game: gameId missing.",
      );
    ...
    try {
      ...
      if (!currentGameState || currentGameState.gameId !== gameId) {
        ...
        if (currentGameState) {
          ...
          logger.warn(
            ...
            "request_start_game: Game not found in memory or DB.",
          );
          socket.emit(GAME_EVENTS.ACTION_ERROR, {
      ...
      const requestingPlayerRole = getRoleBySocketId(
        currentGameState,
        socket.id,
      );
      if (!requestingPlayerRole) {
        logger.warn(
          { socketId: socket.id, gameId },
          "request_start_game: Requesting user not found in this game or has no role.",
        );
        socket.emit(GAME_EVENTS.ACTION_ERROR, {
          message:
            "Cannot start game: Your player role is not recognized for this game. Please rejoin.",
          event: "request_start_game",
        });
        return ack({
          status: "error",
      ...
      const attemptResult = attemptToStartGame(
        currentGameState,
        requestingPlayerRole,
      );
      ...
      if (attemptResult.success === false) {
        // attemptToStartGame signals failure
        logger.warn(
          {
            socketId: socket.id,
            role: requestingPlayerRole,
            gameId,
      ...
      if (attemptResult.updatedGameState.gamePhase === GAME_PHASES.DEALING) {
        logger.info(
          { gameId, currentPhase: attemptResult.updatedGameState.gamePhase },
          "Game phase is DEALING, proceeding to deal hands.",
        );
        try {
          const stateAfterDealing = startNewHand(
            attemptResult.updatedGameState,
          ); // startNewHand is pure
          ...
          logger.info(
            { gameId, newPhase: stateAfterDealing.gamePhase },
            `Game started and hands dealt. Broadcasting updated state.`,
          );
          ...
          logger.error(
            { err: snhError, gameId },
            `Error during startNewHand after request_start_game.`,
          );
          ...
          await gameRepository.updateGame(
            gameId,
            attemptResult.updatedGameState,
          ); // Persist to DB (Layer 5)
          socket.emit(GAME_EVENTS.ACTION_ERROR, {
            message: `Failed to deal cards: ${snhError.message}`,
            event: "request_start_game",
          });
          io.to(gameId).emit(
            GAME_EVENTS.STATE_UPDATE,
            attemptResult.updatedGameState,
          ); // Inform others of the state before dealing
          return ack({
            status: "error",
            message: `Failed to deal cards: ${snhError.message}`,
          });
        ...
        logger.warn(
          {
            socketId: socket.id,
            gameId,
            resultingPhase: attemptResult.updatedGameState.gamePhase,
          },
          "Game start requested, but phase not set to DEALING. Conditions possibly not fully met or phase logic changed.",
        );
        ...
        socket.emit(GAME_EVENTS.ACTION_ERROR, {
          message:
            ...
            "Could not start the game (conditions not met).",
          event: "request_start_game",
        });
        return ack({
          status: "error",
      ...
      logger.error(
        ...
        `Error processing request_start_game for game ${gameId}.`,
      );
      socket.emit(GAME_EVENTS.ACTION_ERROR, {
        message:
          error.message || "An error occurred while trying to start the game.",
        event: "request_start_game",
      });
      return ack({
        status: "error",
  ...
  socket.on(GAME_EVENTS.JOIN_GAME, async (data, ack) => {
    ...
    if (!data || typeof data.playerName !== "string") {
      logger.debug(
        { socketId: socket.id, dataReceived: data },
        "JOIN_GAME: Invalid data - playerName string required.",
      );
      return ack({
        status: "error",
        message: "Invalid request: playerName is required.",
      });
    }
    ...
    logger.debug(
      { socketId: socket.id, dataReceived: data, userId: user.id },
      "JOIN_GAME event received.",
    );
    ...
    try {
      ...
      if (!gameIdToJoin) {
        ...
        logger.info(
          `New game created by ${playerName} (${user.id}) with ID: ${gameIdToJoin}. Player assigned role ${assignedRole}.`,
        );

        socket.join(gameIdToJoin);
        socket.gameId = gameIdToJoin;
        if (socket.request.user) socket.request.user.role = assignedRole;
        else socket.request.user = { role: assignedRole, id: user.id };
        ...
        logger.info(
          `Player ${playerName} (${user.id}) created game ${gameIdToJoin} as ${assignedRole}. State broadcasted.`,
        );
        return ack(null, {
          status: "ok",
          message: "New game created and joined.",
          gameId: gameIdToJoin,
          role: assignedRole,
          players: gameState.players,
        ...
        let currentGameStateFromRepo =
          await gameRepository.getGame(gameIdToJoin);
        if (!currentGameStateFromRepo) {
          logger.warn(
            { gameIdToJoin },
            "JOIN_GAME: Game not found in repository for joining.",
          );
          logger.debug(
            { gameIdToJoin },
            "JOIN_GAME: Sending ack with error - Game not found.",
          );
        ...
        if (gameState.gamePhase !== GAME_PHASES.LOBBY) {
          logger.warn(
            `Player ${playerName} (${user.id}) tried to join active/ended game: ${gameIdToJoin}, Phase: ${gameState.gamePhase}`,
          );
          logger.debug(
            { gameIdToJoin, gamePhase: gameState.gamePhase },
            "JOIN_GAME: Sending ack with error - Game not in lobby phase.",
          );
          return ack({
            status: "error",
            message: `Game ${gameIdToJoin} is not in lobby phase.`,
          });
        }
        ...
        for (const role of PLAYER_ROLES) {
          ...
          if (
            ...
            logger.warn(
              `Player ${playerName} (${user.id}) attempted to join game ${gameIdToJoin} but is already connected as ${role}.`,
            );
            logger.debug(
              { gameIdToJoin, userId: user.id, role },
              "JOIN_GAME: Sending ack with error - Already in game.",
            );
            return ack({
              status: "error",
        ...
        if (existingPlayerRole) {
          ...
          if (isLobbyFull(gameState)) {
            logger.warn(
              `Player ${playerName} (${user.id}) tried to join full game (lobby full check): ${gameIdToJoin}`,
            );
            logger.debug(
              { gameIdToJoin, userId: user.id },
              "JOIN_GAME: Sending ack with error - Game is full.",
            );
          ...
          if (!assignedRole) {
            logger.error(
              `Lobby join error: No role available in game ${gameIdToJoin} despite not being full.`,
            );
            logger.debug(
              { gameIdToJoin, userId: user.id },
              "JOIN_GAME: Sending ack with error - No role available.",
            );
            return ack({
              status: "error",
              message:
                "Failed to assign role, lobby might be in an inconsistent state.",
            });
          }
          gameState = assignRoleToPlayer(
            gameState,
        ...
        if (
          isLobbyFull(finalGameStateToSaveAndBroadcast) &&
          finalGameStateToSaveAndBroadcast.gamePhase === GAME_PHASES.LOBBY
        ) {
          logger.info(
            `Lobby for game ${gameIdToJoin} is now full. Starting game automatically.`,
          );
          try {
            const stateAfterAutoStart = startNewHand(
              finalGameStateToSaveAndBroadcast,
            ...
            logger.info(
              `Game ${gameIdToJoin} auto-started by lobby full. Phase: ${finalGameStateToSaveAndBroadcast.gamePhase}`,
            );
            finalAckMessage = "Joined game, and game is now starting.";
          } catch (snhError) {
            logger.error(
              { err: snhError, gameId: gameIdToJoin },
              `Failed to auto-start new hand for game ${gameIdToJoin} after lobby full.`,
            );
            io.to(gameIdToJoin).emit(GAME_EVENTS.ACTION_ERROR, {
              message: `Failed to auto-start game: ${snhError.message}`,
            });
            ...
            return ack(null, {
              status: "ok",
              message: "Joined lobby, but failed to auto-start game.",
              gameId: gameIdToJoin,
              role: assignedRole,
              players: getGameState().players,
              gameState: getGameState(),
            });
          }
        }
        ...
        io.to(gameIdToJoin).emit(
          GAME_EVENTS.STATE_UPDATE,
          finalGameStateToSaveAndBroadcast,
        );
        logger.info(
          `Player ${playerName} (${user.id}) processed for game ${gameIdToJoin} as ${assignedRole}. State broadcasted with phase: ${finalGameStateToSaveAndBroadcast.gamePhase}`,
        );
        return ack(null, {
          status: "ok",
          message: finalAckMessage,
          gameId: gameIdToJoin,
          role: assignedRole,
          players: finalGameStateToSaveAndBroadcast.players,
          gameState: finalGameStateToSaveAndBroadcast,
        });
      }
    } catch (error) {
      logger.error(
        { err: error, socketId: socket.id, gameIdToJoin, playerName },
        `Error processing JOIN_GAME catch block:`,
      );
      logger.debug(
        { err: error, socketId: socket.id, gameIdToJoin, playerName },
        "JOIN_GAME: Sending ack with error from catch block.",
      );
      return ack({
        status: "error",
        message: error.message || "An error occurred while joining the lobby.",
      });
    }
  });
}
