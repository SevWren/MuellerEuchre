  ...
  socket.on(eventName, async (data) => {
    if (!data || !data.gameId || typeof data.decision !== "boolean") {
      logger.warn(
        ...
        `Invalid data for ${eventName}: gameId and boolean decision required.`,
      );
      socket.emit(GAME_EVENTS.ACTION_ERROR, {
        message:
          "Invalid 'go alone' decision data: 'gameId' and 'decision' fields are required.",
        event: eventName,
      });
      return;
    }
    ...
    try {
      ...
      if (!playerRole) {
        logger.warn(
          { socketId: socket.id, gameId, event: eventName },
          `Received event from unassigned socket for this game.`,
        );
        socket.emit(GAME_EVENTS.ACTION_ERROR, {
      ...
      const result = handleGoAloneDecision(
        currentGameState,
        playerRole,
        decision,
      );
      ...
      if (result.success && result.updatedGameState) {
        ...
        logger.info(
          {
            ...
            playerRole,
            decision,
            newPhase: result.updatedGameState.gamePhase,
          },
        ...
        logger.warn(
          { socketId: socket.id, playerRole, gameId, reason: result.message },
      ...
      socket.emit(GAME_EVENTS.ACTION_ERROR, {
        message: error.message || `Error processing your 'go alone' decision.`,
        event: eventName,
      });
    }
  });
