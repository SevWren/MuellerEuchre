...
*   **State Hydration on Server Restart:** This module does not automatically re-hydrate its `activeGames` map from the database on server startup. This is a known limitation for production reliability as required by **`NFR4.3 (Recovery on Restart)`**. A future `initializeStateFromDb()` function will be required. This function, called from `server.js` at startup, would query `gameRepository.js` for all non-`GAME_OVER` games and populate the `activeGames` map, allowing for seamless recovery after a server restart.