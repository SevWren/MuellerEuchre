...
describe("SocketService", () => {
        ...
        return {
          emit: (event, data, ack) => {
            ...
            if (ack) {
              setTimeout(
                () =>
                  ack(null, { status: "ok", data: { success: true, event } }),
                0,
              );
            }
          },
  ...
  describe("Constructor and Initialization", () => {
    ...
    it("should initialize event listeners on the socket", () => {
      ...
      expect(mockSocket.on).to.have.been.calledWith(
        GAME_EVENTS.ASSIGN_ROLE,
        sinon.match.func,
      );
      expect(mockSocket.on).to.have.been.calledWith(
        GAME_EVENTS.GAME_FULL,
        sinon.match.func,
      );
      expect(mockSocket.on).to.have.been.calledWith(
  ...
  describe("_emitWithAck", () => {
    it("should resolve on successful server acknowledgement", async () => {
      ...
      socketService.socket.timeout = () => ({
        // Ensure this specific test uses a controlled emit
        emit: (evt, pld, ack) => {
          if (evt === event)
            ack(null, { status: "ok", data: { info: "Success!" } });
        },
      });
    ...
    it("should reject on server error acknowledgement", async () => {
      ...
      socketService.socket.timeout = () => ({
        emit: (evt, pld, ack) => {
          if (evt === event)
            ack(null, { status: "error", message: "Server error response" });
        },
      });
      try {
    ...
    it("should reject on timeout", async () => {
        ...
        return {
          emit: (evt, pld, ack) => {
            ...
            setTimeout(() => {
              ...
              ack(
                new Error(
                  `Timeout: Server did not acknowledge ${evt} within ${duration}ms`,
                ),
                null,
              );
      ...
      try {
        ...
        expect(error).to.include(
          `Timeout: Server did not acknowledge ${event}`,
        );
      }
    });
  ...
  describe("Event Handlers", () => {
    it("handleAssignRole should call stateService methods", () => {
      ...
      expect(mockStateService.setGameDetails).to.have.been.calledOnceWith({
        gameId: "g1",
        isHost: true,
      });
      expect(mockStateService.setPlayerId).to.have.been.calledOnceWith(
        "pNorth",
      );
      expect(mockStateService.setPlayerRole).to.have.been.calledOnceWith(
        "north",
      );
      expect(mockStateService.updatePlayerList).to.have.been.calledOnceWith([]);
      expect(mockUiService.displayAssignedRole).to.have.been.calledOnceWith(
        "north",
      );
    });
    ...
    it("handleGameStateUpdate should call stateService.updateFullGameState", () => {
      ...
      expect(mockStateService.updateFullGameState).to.have.been.calledOnceWith(
        newState,
      );
    });
    ...
    it("handleGenericError should call uiService.displayGlobalError", () => {
      ...
      expect(mockUiService.displayGlobalError).to.have.been.calledOnceWith(
        errorData.message,
      );
    });
    ...
    it("handlePlayerAlreadyInGame should call uiService.promptForRejoin", () => {
      ...
      expect(mockUiService.displayMessage).to.have.been.calledOnceWith(
        data.message,
      );
      expect(mockUiService.promptForRejoin).to.have.been.calledOnceWith(
        data.gameId,
      );
    });
    ...
    describe("Socket Connection Event Handlers", () => {
      ...
      beforeEach(() => {
        ...
        const connectCall = mockSocket.on
          .getCalls()
          .find((call) => call.args[0] === "connect");
        if (connectCall) connectHandler = connectCall.args[1];
        else throw new Error("connect handler not found");

        const disconnectCall = mockSocket.on
          .getCalls()
          .find((call) => call.args[0] === "disconnect");
        if (disconnectCall) disconnectHandler = disconnectCall.args[1];
        else throw new Error("disconnect handler not found");
      });
      ...
      it('on "connect" should handle failed rejoin attempt', async () => {
        // Simulate a failed rejoin attempt
        mockStateService.hasReconnectInfo.returns(true); // Simulate reconnect info exists
        sinon
          .stub(socketService, "_emitWithAck")
          .rejects(new Error("Rejoin failed"));
        ...
        expect(
          mockUiService.showReconnectionFailedModal,
        ).to.have.been.calledOnceWith("Rejoin failed"); // Show reconnection failed modal
        socketService._emitWithAck.restore();
      });

      it('on "connect" should handle no reconnect info', async () => {
        // Simulate a fresh connect without reconnect info
      ...
      it('on "disconnect" should call uiService.showConnectionLostMessage', () => {
        ...
        expect(
          mockUiService.showConnectionLostMessage,
        ).to.have.been.calledOnceWith("Server unavailable");
      });
    });
  });
});
