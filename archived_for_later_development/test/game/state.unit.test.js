...
describe("updateGameState()", () => {
  ...
  it("should throw an error and log if state is not initialized", async () => {
    ...
    expect(() => uninitializedStateModule.updateGameState(() => ({}))).to.throw(
  ...
  it("should throw an error and log if updater is not a function", async () => {
    ...
    expect(() =>
      stateModuleWithLogger.updateGameState("not a function"),
    ).to.throw("Updater must be a function.");
    expect(errorSpy.calledOnce).to.be.true;
    expect(errorSpy.calledWith("updateGameState: updater must be a function."))
      .to.be.true;
  });
  ...
  it("should throw an error and log if updater does not return an object", async () => {
    ...
    stateModuleWithLogger.resetFullGame();
    expect(() => stateModuleWithLogger.updateGameState(() => null)).to.throw(
      "Updater function must return a valid new state object.",
    );
    expect(errorSpy.calledOnce).to.be.true;
    expect(
      errorSpy.calledWith(
  ...
  it("should throw an error if the updater function creates a circular reference", async () => {
    ...
    expect(thrownError)
      ...
      .that.includes("Failed to deep clone object");
...
describe("createInitialGameState()", () => {
  ...
  it("should create a new game state object with default values", () => {
    ...
    expect(initialState.tricksTaken).to.deep.equal({
      [TEAMS.TEAM_NS]: 0,
      [TEAMS.TEAM_EW]: 0,
    });
    expect(initialState.teamScores).to.deep.equal({
      [TEAMS.TEAM_NS]: 0,
      [TEAMS.TEAM_EW]: 0,
    });
