...
// Mock the safeStorage module with in-memory implementation
...
// Event handlers for socket events (will be scoped to test suite)
...
describe('StateSyncService', function() {
    ...
    beforeEach(async () => {
        ...
        const module = await esmock(
            ...
            {
                // Mock logger with named exports only
                '../../utils/logger.js': {
                    __esModule: true,
                    log: mockLogger.log,
                    setDebugLevel: mockLogger.setDebugLevel
                },
                // Mock constants with all required exports
                '../../config/constants.js': {
                    __esModule: true,
                    ...mockConstants
                },
                // Mock safeStorage as a default export
                '../../utils/safeStorage.js': {
                    __esModule: true,
                    default: mockSafeStorage
                }
            },
            {
                // Ensure we're not trying to load actual files
                virtual: true
            }
        );
    ...
    afterEach(() => {
        ...
        if (mockSocketService) {
            mockSocketService.off.resetHistory();
            mockSocketService.on.resetHistory();
            mockSocketService.emit.resetHistory();
        }
        
        // Reset all mock safe storage functions
        mockSafeStorage.getItem.resetHistory();
        mockSafeStorage.setItem.resetHistory();
        mockSafeStorage.removeItem.resetHistory();
        
        // Reset mock logger
        mockLogger.log.resetHistory();
        mockLogger.setDebugLevel.resetHistory();
    });
